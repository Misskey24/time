...
- **灵动岛**：可选开启实时活动，在支持灵动岛的设备上显示 `HH:MM:SS` 当前时间
- **签名**：无需付费开发者账号，构建产物已配置为巨魔可直接安装

## 系统要求

- iOS 15.0 或更高（iOS 14 可安装但悬浮窗功能无效，因为系统不支持自定义内容画中画）
- 灵动岛时间需要 iOS 16.1 或更高，并且设备本身支持灵动岛；不支持的设备仍可使用锁屏实时活动
- 已安装巨魔 (TrollStore)
...
3. 打开 **显示时间到灵动岛** 开关，可在灵动岛 / 锁屏实时活动里显示 `HH:MM:SS`
4. 点 **启动悬浮窗** → 切到其他 App → 时间窗口悬浮显示
5. 手指拖动可移动；把悬浮窗向左/右边缘划过去，会自动收成侧边小条；再点小条恢复
6. 如果发现时间漂移，回到 App 点 **重新校时**
...
**Q: 打开灵动岛开关后没有出现在灵动岛**
A: 需要 iOS 16.1+，并且是带灵动岛的机型。系统可能先显示为锁屏实时活动；进入后台、锁屏或有其他系统状态变化时才会进入灵动岛形态。

**Q: 灵动岛里为什么没有十分之一秒**
A: 灵动岛属于系统实时活动，刷新频率受 iOS 控制，适合显示秒级时间；十分之一秒仍保留在 App 主界面和 PiP 悬浮窗里。
...
| `Sources/LiveActivityController.swift` | 灵动岛 / 实时活动开关、启动、刷新和结束逻辑 |
...
| `Shared/TimeLiveActivityAttributes.swift` | 主 App 与 Widget 扩展共享的实时活动数据结构 |
| `Sources/TimeDynamicIslandExtension/TimeDynamicIslandWidget.swift` | 灵动岛、锁屏实时活动和紧凑态 UI |
